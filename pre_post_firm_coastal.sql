-- 1.1 SUMMARY TABEL FOR PRE-FIRM CLAIMS in coastal flood V zone
drop table summary.prefirm_claim_state_jurisdiction_2015v;
create table summary.prefirm_claim_state_jurisdiction_2015v as
with s as (
  select
...
alter table summary.prefirm_claim_state_jurisdiction_2015v add primary key (year, state, community, jurisdiction_id);

-- 1.2 SUMMARY TABEL FOR POST-FIRM CLAIMS in coastal flood V zone
drop table summary.postfirm_claim_state_jurisdiction_2015v;
create table summary.postfirm_claim_state_jurisdiction_2015v as
with s as (
  select
  extract(year from dt_of_loss) as year,
  re_state as state,
...
alter table summary.postfirm_claim_state_jurisdiction_2015v add primary key (year, state, community, jurisdiction_id);

------------------------------------------------------------------------------------------------------------------------------------------------
-- 2.1 SUMMARY TABLE FOR PRE-FIRM POLICY in coastal flood V zone
drop table summary.prefirm_policy_state_jurisdiction_2015v;
create table summary.prefirm_policy_state_jurisdiction_2015v as
with s as (
  select
...
alter table summary.prefirm_policy_state_jurisdiction_2015v add primary key (year, state, community, jurisdiction_id);

-- 2.2 SUMMARY TABLE FOR POST-FIRM POLICY in coastal flood V zone
drop table summary.postfirm_policy_state_jurisdiction_2015v;
create table summary.postfirm_policy_state_jurisdiction_2015v as
with s as (
  select
  extract(year from end_eff_dt) as year,
  re_state as state,
...
alter table summary.postfirm_policy_state_jurisdiction_2015v add primary key (year, state, community, jurisdiction_id);

------------------------------------------------------------------------------------------------------------------------------------------------

-- 3. Policy and Claim summary by state and by year
drop table us.firm_claim_policy_state_year_v;
create table us.firm_claim_policy_state_year_v as
with cpre as(
  select
  state,
  year,
  sum(count) as count,
  sum(pay) as pay
  from summary.prefirm_claim_state_jurisdiction_2015v
  group by 1,2
  order by 1,2),
cpost as(
  select
  ...
  sum(count) as count,
  sum(pay) as pay
  from summary.postfirm_claim_state_jurisdiction_2015v
  group by 1,2
  order by 1,2),
ppre as(
  select
  ...
  from summary.prefirm_policy_state_jurisdiction_2015v
  group by 1,2
  order by 1,2),
ppost as(
  select
  ...
  sum(count) as count,
  sum(premium) as premium
  from summary.postfirm_policy_state_jurisdiction_2015v
  group by 1,2
  order by 1,2)
select
...
alter table us.firm_claim_policy_state_year_v add primary key (state,year);
...
drop table us.firm_ratio_state_v;
create table us.firm_ratio_state_v as
with a as (
  select
  state,
  ...
  from us.firm_claim_policy_state_year_v
...
alter table us.firm_ratio_state_v add primary key (state);
